...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const rows = await db<{ storage_path: string; filename: string; mime_type: string }[]>`
    SELECT storage_path, filename, mime_type FROM user_files
  ...
  try {
    const r = await fetch(`${BRIDGE_URL}/v1/files/read?path=${encodeURIComponent(rows[0].storage_path)}`, {
    ...
    return new Response(r.body, { headers: { "Content-Type": rows[0].mime_type, "Content-Disposition": `inline; filename="${encodeURIComponent(rows[0].filename)}"` } });
  } catch (e) {
